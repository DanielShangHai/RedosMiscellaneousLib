...
#include <boost/log/detail/log_fwd.hpp>
...
// FIXME use BOOST_STATIC_CONSTANT
...
// alias...
BOOST_LOG_DEFINE_LEVEL(default_level, 1000)
...
struct enabled_logger {
    ...
    enabled_logger(logger & l, const force_enabled &) : m_stream(new logging_types::stream), m_logger(l), m_level(0) {
...
template<> struct logger_keeper_is_enabled<true,true> {
    bool is_enabled(logger & , level_type ) const { return true; }
};
template<> struct logger_keeper_is_enabled<true,false> {
    bool is_enabled(logger & , level_type ) const { return false; }
};
...
// #define BOOST_LOG(log_name) if(true) ; else (std::cout)
