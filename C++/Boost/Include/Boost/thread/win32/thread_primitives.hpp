...
namespace boost
{
    namespace detail
    {
        namespace win32
        {
            ...
            inline handle create_anonymous_mutex( event_type, initial_event_state state )
            {
#if !defined(BOOST_NO_ANSI_APIS)  
                handle const res=win32::CreateMutexA(0, state, 0);
#else
                handle const res=win32::CreateMutexW(0, state, 0);
#endif                
            ...
            inline void release_mutex( handle mtx )
            {
                BOOST_VERIFY(ReleaseMutex( mtx )!=0);
            }
